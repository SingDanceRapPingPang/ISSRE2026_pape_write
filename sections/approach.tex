...
\cite{leue2019clock}. Each modifiable numerical bound $b_s$ in a
...
extends it to the multi-property setting \cite{leue2019clock}. Given
a timed diagnostic trace
\begin{equation}
\begin{aligned}
  \tau ={}&
...
\begin{equation}
\begin{aligned}
  &\operatorname{Path}(\tau,\theta,\mathbf{z}_{\tau}) =
  \mathit{Init}(\nu_0) \\
  &\quad{}\land
  \bigwedge_{i=0}^{n-1}
  \Bigl(
...
$\theta$. The original violation encoding can then be viewed as the
conjunction of path executability and the final property violation:
\begin{equation}
...
skeleton, the single-trace repair obligation uses two copies of the
trace variables:
\begin{equation}
\begin{aligned}
  &\Psi_{\tau,\varphi}^{\mathrm{exec}}(\theta)
  =
  \operatorname{QE}\Bigl(
...
\cite{leue2019clock}. Traces sharing identical discrete transition
...
equivalence \cite{leue2019clock}. This step ensures that clock-bound

...
We check $\varphi_1$,
\texttt{A[] (Controller.Engage imply cut >= 2)}, and $\varphi_2$,
\texttt{A[] not Controller.Late}. In the faulty controller, the
\texttt{Cut} invariant $\mathit{cut}\leq 1$ and the
\texttt{Cut}$\to$\texttt{Engage} guard $\mathit{cut}\geq 1$ allow
engagement after one time unit, violating $\varphi_1$. A single-property
repair raises both $\mathit{cut}$ bounds to $2$. If the \texttt{Engage}
invariant $\mathit{mesh}\leq 3$ and the
\texttt{Engage}$\to$\texttt{Ready} guard $\mathit{mesh}\geq 3$ remain
unchanged, the controller cannot return to \texttt{Ready} until
$2+3=5$ time units after the request. It can therefore reach
\texttt{Controller.Late} and violate $\varphi_2$. A multi-property repair
must therefore adjust both $\mathit{cut}$ and $\mathit{mesh}$ while
...
appropriate. This admissibility criterion prevents repairs that satisfy
...
property-trace constraints across refinement rounds. A bound edit that
...
candidate only after full-property regression verification and an
